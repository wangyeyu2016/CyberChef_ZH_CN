...
class Typex extends Operation {
    ...
    constructor() {
        ...
        this.description = "<br> <br> Typex最初是由英国皇家空军在第二次世界大战之前建造的，它是基于第二次世界大战的机器，其基于谜团，包括对五个带有更多垫脚点和更多垫脚的转子，可互换的接线芯。它在英国和英联邦军队中使用。产生了许多以后的变体；在这里，我们模拟了WW2 ERA Mark 22 Typex，并带有用于反射器和输入的插件。定期更改Typex转子，没有公开：提供一个随机示例集。<br> <br>要配置反射器插件，在反射器框中输入一对连接的字母字符串，例如<code> ab cd ef </code>将A连接到B，C到D，并将E连接到F（您需要连接每个字母）。还有一个输入插件：与Enigma的插头不同，它不仅限于对，因此它像转子一样输入（无需步进）。要创建自己的转子，请输入转子映射a至z到z到的字母，然后选择<code>＆lt; </code>，然后是阶梯点的列表。<br> <br>更详细的描述在这里可以找到Enigma，Typex和Bombe操作<a href='https://github.com/gchq/cyberchef/wiki/wiki/enigma/enigma-the-bas-bes-the-bombe-and-typex'>在这里可以找到</a>。";
